...
section "Prefixes are defined of type 'd here"
...
using Complete_subset_\<pi> LocalLexing.Complete_def LocalLexing_axioms by fastforce
...
definition Prefixes :: "'d list \<Rightarrow> 'd list set"
where
  ...
  case less
    ...
    proof (induct rule: disjCases2)
      case 1
        ...
        have z_in_Scan_y: "?z \<in> Scan T k {?y}" 
          ...
          using next_symbol_y apply auto apply(simp add: chars_of_token_def)
          done
        from pvalid_y have "?y \<in> Gen(Prefixes p)"
          apply (simp add: Gen_def)
          apply (rule_tac x=p in exI)
          by (auto simp add: paths_le_def p_dom chars_of_token_def)
    ...
    next
      case 2
        ...
        proof -
          have f1: "\<forall>ss ssa ssb. \<not> is_prefix (ss::('a, 'b) symbol list) (ssa @ ssb) \<or> is_prefix ss ssa \<or> (\<exists>ssc. ssc \<noteq> [] \<and> is_prefix ssc ssb \<and> ss = ssa @ ssc)"
            by (simp add: is_prefix_of_append)
          have f2: "\<And>ss ssa. is_prefix ((ss::('a, 'b) symbol list) @ ssa) ss \<or> \<not> is_prefix ssa []"
        ...
        have x1_in_bin: "x1 \<in> bin I (item_origin z)"
          using bin_def x1 x1_in_I z12 by fastforce           
        have x2_in_Complete: "x2 \<in> Complete k' I"
          ...
          using  \<alpha>_decompose is_complete_def item_nonterminal_def next_symbol_def x1 z12 apply fastforce
          using is_complete_def z12 apply fastforce
          using x1 next_symbol_def item_nonterminal_def z12 \<alpha>_decompose 
        proof -
          from x1 is_complete_def  have "item_rhs x1 = \<alpha>@\<beta>" by simp
          with x1 is_complete_def \<alpha>_decompose have inc:"\<not> is_complete x1" by force
          from  x1 \<alpha>_decompose have "item_rhs x1 ! item_dot x1 = A" by auto
          with inc next_symbol_def x1 have some:"next_symbol x1 = Some (A)" by metis
          from z  item_rule_def have "item_rule z = (A, w)" by auto
          with item_nonterminal_def  fst_def have "fst (item_rule z) = A" by simp
          with item_nonterminal_def have "item_nonterminal z = A" by blast
          with some show  "next_symbol x1 = Some (item_nonterminal z)" by simp
        qed
