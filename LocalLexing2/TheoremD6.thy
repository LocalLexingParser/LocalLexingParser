...
definition inc_dot :: "nat \<Rightarrow>  ('a, 'b) item \<Rightarrow>  ('a, 'b) item"
where
...
lemma Predict_mk_regular1: 
  "\<exists> (P ::  ('a, 'b) rule \<Rightarrow>  ('a, 'b) item \<Rightarrow> bool) F. Predict k = mk_regular1 P F"
proof -
  let ?P = "\<lambda> r x:: ('a, 'b) item. r \<in> \<RR> \<and> item_end x = k \<and> next_symbol x = Some(fst r)"
  let ?F = "\<lambda> r (x::  ('a, 'b) item). init_item r k"
  show ?thesis
...
lemma Complete_mk_regular2: 
  "\<exists> (P :: dummy \<Rightarrow>  ('a, 'b) item \<Rightarrow>  ('a, 'b) item \<Rightarrow> bool) F. Complete k = mk_regular2 P F"
proof -
...
lemma Scan_mk_regular1:
  "\<exists> (P ::  ('a, 'b, 'c) token \<Rightarrow>  ('a, 'b) item \<Rightarrow> bool) F. Scan T k = mk_regular1 P F"
proof -
  let ?P = "\<lambda> (tok:: ('a, 'b, 'c)token) (x:: ('a, 'b)item). item_end x = k \<and> tok \<in> T \<and> next_symbol x = Some (fst tok)"
  let ?F = "\<lambda> (tok:: ('a, 'b, 'c)token) (x:: ('a, 'b)item). inc_item x (k + length (snd tok))"
  show ?thesis
    apply (rule_tac x="?P" in exI)
    apply (rule_tac x="?F" in exI)
    apply (rule_tac ext)
  ...
  case less
    ...
    next
      case 2
        ...
        have "next_symbol x = Some t \<Longrightarrow> \<exists>\<delta>. item_\<beta> x = t # \<delta>" using less.prems(1)  next_symbol_starts_item_\<beta> by blast
        have "is_complete x \<Longrightarrow> item_\<beta> x = []" using less.prems(1)  wellformed_complete_item_\<beta>  by blast
        then have "\<not>(item_\<beta> x = []) \<Longrightarrow> \<not> (is_complete x)" by blast 
        with less.prems(2) N have "\<not> is_complete x" by simp
        then have "next_symbol x  = Some (item_rhs x ! item_dot x)" using next_symbol_def by metis
        then have next_symbol_x: "next_symbol x = Some N"
          using N less.prems(1) less.prems(2)  next_symbol_starts_item_\<beta> 
              by fastforce
