...
definition produced_by_scan_step :: "('a, 'b) item \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> bool" where
            ...
            next
              case 2
                ...
                proof - (* automatically generated *)
                  have f1: "\<forall>f I Ia i. (\<not> mono f \<or> \<not> (I::('a, 'b)item set) \<subseteq> Ia \<or> (i::('a, 'b)item) \<notin> f I) \<or> i \<in> f Ia"
                    by (meson mono_subset_elem)
                  obtain ii :: "('a, 'b) item set \<Rightarrow> ('a, 'b) item set \<Rightarrow> ('a, 'b)item" where
