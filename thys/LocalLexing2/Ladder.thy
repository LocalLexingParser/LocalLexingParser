...
(*LeftDerivationFix describes a left derivation
i j describing the same symbol, along which derivation D can be successfully split*)
definition LeftDerivationFix :: "('a, 'b) sentence \<Rightarrow> nat \<Rightarrow> ('a, 'b) derivation \<Rightarrow> nat \<Rightarrow> ('a, 'b)  sentence \<Rightarrow> bool"
where
...
(*inductive case if leftderivationfix, where leftderivationfix is a child of the first 
derivation (ix yields a split along the newly introduced rule*)
definition LeftDerivationIntro :: 
  "('a, 'b) sentence \<Rightarrow> nat \<Rightarrow> ('a, 'b) rule \<Rightarrow> nat \<Rightarrow> ('a, 'b)derivation \<Rightarrow> nat \<Rightarrow> ('a, 'b)sentence \<Rightarrow> bool"
where
...
(*LeftDerivationFix is essentially a rule with multiple children*)
...
(*splits at \<Longrightarrow> whether delta is composed of \<alpha>N\<beta>*)
lemma LeftDerives1_X_is_part_of_rule[consumes 2, case_names Suffix Prefix]:
  ...
  (*not possible to apply rule on suffix*)
...
(*lemma to add add an additional derivation to d while "shrinking" the suffix*)
...
(*each LeftDerivation is a LeftDerivationFix or LeftDerivationIntro*)
...
(*a ladder with regards to a derivation is valid if
all ladder n are smaller then D
2: monotonic ladder n and absolute last n is = length D*)
definition is_ladder :: "('a, 'b) derivation \<Rightarrow> ladder \<Rightarrow> bool" where
...
(*gives in between word after ladder_n index derivations applications*)
definition ladder_\<gamma> :: "('a, 'b) sentence \<Rightarrow> ('a, 'b) derivation \<Rightarrow> ladder \<Rightarrow> nat \<Rightarrow> ('a, 'b) sentence" where
  "ladder_\<gamma> a D L index = Derive a (take (ladder_n L index) D)"
(*word right before ladder_n[i] th derivation application*)
definition ladder_\<alpha> :: "('a, 'b) sentence \<Rightarrow> ('a, 'b) derivation \<Rightarrow> ladder \<Rightarrow> nat \<Rightarrow> ('a, 'b) sentence" where
  "ladder_\<alpha> a D L index = (if index = 0 then a else ladder_\<gamma> a D L (index - 1))"

(*index at ladder causes a LeftDerivation Intro with the other values of the same step*)
definition LeftDerivationIntrosAt :: "('a, 'b) sentence \<Rightarrow> ('a, 'b) derivation \<Rightarrow> ladder \<Rightarrow> nat \<Rightarrow> bool" where
...
(*remark aren't i and ix the same*)

(*ladder that only describes LeftDerivationIntros*)
definition LeftDerivationIntros :: "('a, 'b) sentence \<Rightarrow> ('a, 'b) derivation \<Rightarrow> ladder \<Rightarrow> bool" where
  "LeftDerivationIntros a D L = (
     \<forall> index. 1 \<le> index \<and> index < length L \<longrightarrow> LeftDerivationIntrosAt a D L index)"

(*effectively one root (index 0) which is a LeftDerivationFix, i.e. "independent", all others are only intros*)
definition LeftDerivationLadder :: "('a, 'b) sentence \<Rightarrow> ('a, 'b) derivation \<Rightarrow> ladder \<Rightarrow>('a, 'b) sentence \<Rightarrow> bool" where 
...
(*possibility to use this for implication of TreeDerivs*)
...
(*growing a ladder by adding an intro to the end*)
...
(*look at this more closely(!) possibly heavy implications for trees*)
...
(*a short ladder by definition entails a fix*)
...
qed    
(*fix by definition are two or more concurrent derivations*)
(*really*)
...
(*probably highly important for proof*)
...
qed 
(*fix is supposed to only work on terminals?*)        
  ...
  proof (induct rule: disjCases2)
    case 1
      ...
      proof - (* automatically found *)
        obtain nn :: "(nat \<times> ('a, 'b) symbol \<times> ('a, 'b) symbol list) list \<Rightarrow> (nat \<times> nat \<times> nat) list \<Rightarrow> nat" and 
          nna :: "(nat \<times> ('a, 'b) symbol \<times> ('a, 'b) symbol list) list \<Rightarrow> (nat \<times> nat \<times> nat) list \<Rightarrow> nat" and 
          nnb :: "(nat \<times> ('a, 'b)symbol \<times> ('a, 'b) symbol list) list \<Rightarrow> (nat \<times> nat \<times> nat) list \<Rightarrow> nat" where
...
proof -
  ...
  {
    fix i :: nat
    fix r :: "('a, 'b) symbol \<times> (('a, 'b) symbol list)"
...
(*actually usable for tree rotation*)
...
definition ladder_stepdown_\<alpha>_0 :: "('a, 'b) sentence \<Rightarrow> ('a, 'b)derivation \<Rightarrow> ladder \<Rightarrow> ('a, 'b) sentence" where
...
proof -
  ...
  have derive_derive: "Derive \<alpha> (take (Suc (ladder_n L 0)) D) = 
    ...
    proof -
      ...
      obtain sss :: "('a, 'b) symbol list \<Rightarrow> (nat \<times> ('a, 'b) symbol \<times> ('a, 'b) symbol list) list \<Rightarrow> (nat \<times> ('a, 'b)symbol \<times> ('a, 'b)symbol list) list \<Rightarrow> ('a, 'b)symbol list \<Rightarrow> ('a, 'b)symbol list" where
...
proof -
  ...
  proof -
    ...
    then have \<beta>_def: "\<beta> = ladder_stepdown_\<alpha>_0 \<alpha> D L"
      proof -
        obtain sss :: "nat \<Rightarrow> ('a, 'b) symbol list \<Rightarrow> ('a, 'b) symbol list" and ss :: "nat \<Rightarrow> ('a, 'b) symbol list \<Rightarrow> ('a, 'b) symbol" and sssa :: "nat \<Rightarrow> ('a, 'b) symbol list \<Rightarrow> ('a, 'b) symbol list" where
