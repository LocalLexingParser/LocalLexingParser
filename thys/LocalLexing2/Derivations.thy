...
definition leftderives1 :: "('a, 'b)sentence \<Rightarrow> ('a,'b) sentence \<Rightarrow> bool"
where
  "leftderives1 u v = 
     (\<exists> x y N \<alpha>. 
          u = x @ [N] @ y
...
definition leftderivations1 :: "(('a, 'b) sentence \<times> ('a, 'b) sentence) set"
where
...
definition leftderivations :: "(('a, 'b) sentence \<times> ('a, 'b) sentence) set"
where 
...
definition leftderives :: "('a, 'b) sentence \<Rightarrow> ('a, 'b) sentence \<Rightarrow> bool"
where
...
definition is_leftderivation :: "('a, 'b) sentence \<Rightarrow> bool"
where
...
definition Derives1 :: "('a, 'b) sentence \<Rightarrow> nat \<Rightarrow> ('a, 'b) rule \<Rightarrow> ('a, 'b) sentence \<Rightarrow> bool"
where
  "Derives1 u i r v = 
     (\<exists> x y N \<alpha>. 
          u = x @ [N] @ y
...
fun Derivation :: "('a, 'b) sentence \<Rightarrow> ('a, 'b) derivation \<Rightarrow> ('a, 'b) sentence \<Rightarrow> bool"
where
  "Derivation a [] b = (a = b)"
...
definition leftmost :: "nat \<Rightarrow> ('a, 'b) sentence \<Rightarrow> bool"
where
...
lemma rule_\<alpha>_type[simp]: "(N, \<alpha>) \<in> \<RR> \<Longrightarrow> is_sentence \<alpha>"
  apply (insert validRules)
  apply (auto simp add: is_sentence_def is_symbol_def list_all_iff is_terminal_def is_nonterminal_def split: sum.splits)
  done
...
lemma is_symbol_distinct: "is_symbol x \<Longrightarrow> is_terminal x \<noteq> is_nonterminal x"
  ...
  apply (auto simp add: is_symbol_def is_terminal_def is_nonterminal_def split: sum.splits)
  done
...
proof -
  ...
  from assms have J1:"i \<in> ?J"
    ...
    by (metis is_nonterminal_def rule_nonterminal_type)
...
lemma leftmost_cons_terminal: 
  ...
  apply(auto simp add: leftmost_def)
  apply(auto simp add: take_Cons' is_terminal_nonterminal nth_Cons' not_gr0)
(* nontermination under new type *)
using Suc_diff_1 Suc_less_eq is_terminal_nonterminal is_word_def leftmost_def length_Cons 
    list_all_simps(1)  nth_Cons' take_Cons' apply(auto)
   apply fastforce
  using not_le apply fastforce
  apply (smt (z3) One_nat_def list_all_simps(1))
...
proof -
  ...
  have f4: "\<And>n ss ssa. take (length (take n (ss::('a, 'b)symbol list))) (ssa:: ('a, 'b) symbol list) = take (length ss) (take n ssa)"
    by (metis length_take take_take)
  have f5: "\<And>ss. take j (ss::('a, 'b) symbol list) = take i (take j ss)"
  ...
  have f8: "\<And>n ss. n = length (take n (ss::('a, 'b)symbol list)) \<or> length ss < n"
    by (metis leI length_take min.absorb2)
  have f9: "\<And>ss. take j (ss:: ('a, 'b) symbol list) = take j (take i ss)"
    using f7 f6 f5 by (metis take_take)
  have f10: "\<And>ss n. length (ss::('a, 'b) symbol list) \<le> n \<or> length (take n ss) = n"
    using f8 by (metis dual_order.strict_implies_order)
  then have f11: "\<And>ss ssa. length (ss::('a, 'b)symbol list) = length (take (length ss) (ssa::('a, 'b)symbol list)) \<or> length (take (length ssa) ss) = length ssa"
    by (metis length_take min.absorb2)
  have f12: "\<And>ss ssa n. take (length (ss::('a, 'b)symbol list)) (ssa::('a, 'b)symbol list) = take n (take (length ss) ssa) \<or> length (take n ss) = n"
    using f10 by (metis min.absorb2 take_take)
  { assume "\<not> j < j"
    { assume "\<not> j < j \<and> i \<noteq> j"
      moreover
      { assume "length a \<noteq> j \<and> length (take i a) \<noteq> i"
        then have "\<exists>ss. length (take (length (take i (take (length a) (ss::('a, 'b) symbol list)))) (take j ss)) \<noteq> length (take i (take (length a) ss))"
          using f12 f11 f6 f5 f4 by metis
        then have "\<exists>ss ssa. take (length (ss::('a, 'b) symbol list)) (take j (ssa::('a, 'b) symbol list)) \<noteq> take (length ss) (take i (take (length a) ssa))"
          using f11 by metis
...
definition derivation_ge :: "('a, 'b) derivation \<Rightarrow> nat \<Rightarrow> bool"
where
...
definition derivation_shift :: "('a, 'b) derivation \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> ('a, 'b) derivation"
where
...
definition LeftDerives1 :: "('a, 'b) sentence \<Rightarrow> nat \<Rightarrow> ('a, 'b) rule \<Rightarrow> ('a, 'b) sentence \<Rightarrow> bool"
where 
...
lemma LeftDerives1_implies_leftderives1: "LeftDerives1 u i r v \<Longrightarrow> leftderives1 u v"
  ...
  by (smt (verit, ccfv_SIG) Derives1_def LeftDerives1_def leftderives1_def leftmost_cons_less leftmost_def leftmost_notword nonword_leftmost_exists)
...
fun LeftDerivation :: " ('a, 'b) sentence \<Rightarrow> ('a, 'b) derivation \<Rightarrow> ('a, 'b) sentence \<Rightarrow> bool"
where
...
next
  ...
  using CFG.LeftDerivation.simps(2) CFG_axioms Derivation.simps(2) 
    LeftDerives1_implies_Derives1 by metis 
qed
...
definition Derive :: "('a, 'b) sentence \<Rightarrow> ('a, 'b) derivation \<Rightarrow> ('a, 'b) sentence"
where 
...
using CFG.Derivation_unique_dest CFG_axioms by fast
