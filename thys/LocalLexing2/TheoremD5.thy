...
definition is_continuation :: "nat \<Rightarrow> nat \<Rightarrow>  ('a, 'b, 'c) tokens \<Rightarrow>  ('a, 'b, 'c)tokens \<Rightarrow> bool" where
  ...
  case (Induct q k u)
    ...
    next
      case 2
        ...
        have "charslength ts' = 0" using charslength_0 is_continuation_def pts ts' by fastforce
...
proof (cases "charslength p \<ge> k")
  case True
    then show ?thesis
      apply (rule_tac x=u in exI)
      using le_neq_implies_less p_dom subsetCE subset_\<P> by fast
next
  case False
    ...
    next
      case 2
        ...
        with k' have "p \<in> \<P> (charslength p) (Suc u')"
          using le_neq_implies_less subsetCE  subset_\<P> by fast
...
lemma wellformed_complete_item_\<beta>:
  ...
  shows "item_\<beta> x = []"
proof -
  from complete have "(length (item_rhs x) \<le> item_dot x)" using is_complete_def by blast
  from this  have "drop (item_dot x) (item_rhs x) = []" by simp
  then show ?thesis using item_\<beta>_def by metis
qed
...
proof -
  from complete have "(length (item_rhs x) \<le> item_dot x)" using is_complete_def by blast
  from this  have "take (item_dot x) (item_rhs x) = item_rhs x" by simp
  then show ?thesis using item_\<alpha>_def  wellformed wellformed_item_derives by metis
qed
...
proof -
  ...
  next
    ...
    then have d1:"is_derivation ((terminals p1)@([item_nonterminal x2] @ \<delta>')@\<delta>)"
      using is_derivation_derives \<delta>' by auto
    have "item_rule x2 \<in> \<RR>" using  p2valid wellformed_item_def by blast
    then have  "(item_nonterminal x2 , item_rhs x2) \<in> \<RR>"  by(simp add:  item_nonterminal_def item_rhs_def)
    from p2valid  have d2:"derives (item_\<alpha> x2) (terminals (drop y p2))" and wf:"wellformed_item x2" by auto
    from x12 have "is_complete x2" by simp
    with wf  have "derives [item_nonterminal x2] (item_\<alpha> x2)" using wellformed_complete_item_derives by blast
    with d2  have "derives [item_nonterminal x2] (terminals (drop y p2))" using derives_trans by fast
    with d1 have "is_derivation ((terminals p1)@(terminals (drop y p2)) @ \<delta>' @\<delta>)"
      using is_derivation_derives  by auto
...
proof -
  ...
  next
    ...
    from p pvalid_def x_is_scan have charslength_p: "charslength p = k"
      using bin_def mem_Collect_eq by fastforce
    ...
    have wellformed_x: "wellformed_item x"
      ...
      using ptok_doc_tokens charslength.simps doc_tokens_length apply fastforce
      apply (simp only: clen_ptok)
      using valid by simp
    have "pvalid (p@[tok]) x"
      apply (auto simp only: pvalid_def)
