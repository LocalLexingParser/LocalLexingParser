...
definition wellformed_token :: "('a, 'b, 'c) token \<Rightarrow> bool"
where
  "wellformed_token t = is_terminal (terminal_of_token t)"

definition wellformed_tokens :: "('a, 'b, 'c) tokens \<Rightarrow> bool"
where
  "wellformed_tokens ts = list_all wellformed_token ts"

definition doc_tokens :: "('a, 'b, 'c) tokens \<Rightarrow> bool"
where
  "doc_tokens p = (wellformed_tokens p \<and> is_prefix (chars p) Doc)"

definition wellformed_item :: "('a, 'b) item \<Rightarrow> bool"
where 
  "wellformed_item x = (
...
definition wellformed_items :: "('a, 'b)items \<Rightarrow> bool"
where
...
(*list of tokens, valid with regard to an item*)
definition pvalid :: "('a, 'b, 'c) tokens \<Rightarrow> ('a, 'b) item \<Rightarrow> bool"
where
...
definition Gen :: "('a, 'b, 'c) tokens set \<Rightarrow> ('a, 'b) items"
where
...
definition pvalid_left :: "('a, 'b, 'c) tokens \<Rightarrow> ('a, 'b)item \<Rightarrow> bool"
where
