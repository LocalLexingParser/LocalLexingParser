...
lemma head_of_item_\<beta>_is_next_symbol: 
  assumes "wellformed_item x" "item_\<beta> x = t#\<delta>"
  shows "next_symbol x = Some t"
proof - 
  have "is_complete x \<Longrightarrow> item_\<beta> x = []" using wellformed_complete_item_\<beta> assms(1) by auto
  with assms(2) have "\<not> (is_complete x)" by auto
  then have "next_symbol x = Some (item_rhs x ! item_dot x)" using next_symbol_def by metis 
  then show ?thesis using next_symbol_starts_item_\<beta> assms(1)  assms(2) by fastforce 
qed
...
using Predict_def bin_def by fastforce
