...
definition items_le :: "nat \<Rightarrow> ('a, 'b) items \<Rightarrow>  ('a, 'b) items"
where
  "items_le k I = { x . x \<in> I \<and> item_end x \<le> k }"

definition items_eq :: "nat \<Rightarrow>  ('a, 'b)items \<Rightarrow>  ('a, 'b)items"
where
  "items_eq k I = { x . x \<in> I \<and> item_end x = k }"

definition paths_le :: "nat \<Rightarrow>  ('a, 'b, 'c) tokens set \<Rightarrow>  ('a, 'b, 'c) tokens set"
where 
  "paths_le k P = { p . p \<in> P \<and> charslength p \<le> k }"

definition paths_eq :: "nat \<Rightarrow>  ('a, 'b, 'c) tokens set \<Rightarrow>  ('a, 'b, 'c) tokens set"
where
...
lemma items_le_\<J>_0_0: "items_le 0 (\<J> 0 0) = \<J> 0 0"
  using LocalLexing.\<J>_0_0_item_end LocalLexing.items_le_def LocalLexing_axioms by fast

lemma paths_le_\<P>_0_0: "paths_le 0 (\<P> 0 0) = \<P> 0 0"
  by (auto simp add: paths_le_def)

definition empty_tokens :: " ('a, 'b, 'c) token set \<Rightarrow>  ('a, 'b, 'c) token set"
where
...
lemma TokensAt_subset_\<X>: "TokensAt k I \<subseteq> \<X> k"
  using TokensAt_def \<X>_def is_terminal_def \<TT>_def by (auto split: sum.splits)
...
proof -
  ...
  have law2:"\<And> I. wellformed_items I \<Longrightarrow> wellformed_items (Predict k I)" using LocalLexing.wellformed_items_Predict LocalLexing_axioms by fast
  then have law0: "\<And> I. wellformed_items I \<Longrightarrow> items_le k (Complete k (Predict k I)) = Complete k ((Predict k (items_le k I)))" using items_le_Complete items_le_Predict by simp 
  then  have law1: "\<And> I. wellformed_items I \<Longrightarrow> items_le k (?g I) = ?h (items_le k I)"
     using LocalLexing.wellformed_items_Predict LocalLexing_axioms  
      items_le_Scan  items_le_Complete by auto
...
proof -
  from LocalLexing.Gen_def LocalLexing_axioms have 1:"Gen P = { x | x p. p \<in> P \<and> pvalid p x }" by fastforce
  from LocalLexing.items_le_def LocalLexing_axioms have  2:"items_le k I = { x . x \<in> I \<and> item_end x \<le> k }" by fastforce
  with 1   pvalid_item_end paths_le_def have "items_le k (Gen P) = { x . x  \<in> (Gen P) \<and> item_end x \<le> k}" using items_le_def by presburger
  with paths_le_def Gen_def show ?thesis
    using pvalid_item_end by auto 
qed
